...
# command handle 
@Client.on_message(filters.command("draw","imagine"))
async def imagine(client, message):
    ...
    url = f"https://horridapi2-0.onrender.com/imagine?prompt={prompt}&api_key={HORRI_API_KEY}"
    async with aiohttp.ClientSession() as session:
        async with session.get(url) as response:
            image_data = await response.read()
    with BytesIO(image_data) as out_file:
        out_file.name = "image.jpg"
        await message.reply_photo(photo=out_file, caption=f"**🎉 Image generated by Movie Provider Bot\n👨‍💻 Join my support group for more awesome content! 💥\nPrompt: {prompt}**")
    await m.delete()
